...
estimate <- cbind.data.frame(EWAS_data1.selected[,5],EWAS_data2.selected[,5])
names(estimate) <- c("CohensF1","CohensF2")
rownames(estimate) <- rownames(EWAS_data1.selected)
corr_ <- cor.test(estimate$CohensF1,estimate$CohensF2,method="pearson")
...
ggplot(estimate, aes(x = CohensF1, y = CohensF2)) + geom_point() + 
  geom_smooth(method = "lm", se = T) + ggtitle(cor.label) + 
  xlab("Cohens f values of top 100 probes in PITT-ADRC")+ylab("Cohens f values of top 100 probes in BDR")
dev.off()
