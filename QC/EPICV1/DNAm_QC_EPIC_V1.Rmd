...
message("    Optional Batches: ",paste(Optional_Batches , collapse = ","))
...
if(!all(mendatory_col %in% colnames(SampleSheet))){
  stop("The following columns are not found in the SampleSheet file:\n",
       paste(setdiff(mendatory_col , colnames(SampleSheet)),collapse = ","))
}
...
QCmetrics <- cbind.data.frame(QCmetrics, Outlier = outliers$outliers)
...
`r sum(outliers$outliers == TRUE)` samples have been picked as being outliers using the outlyx function and have been removed prior to normalisation.
