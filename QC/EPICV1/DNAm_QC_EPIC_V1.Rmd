...
output: 
  html_document:
    ...
    toc_float: true
    includes:
        before_body: /lustre/projects/Research_Project-191391/Morteza/KCL_FACS/Scripts/QC/EPICV1/DNAm_QC_Header.html
        after_body: /lustre/projects/Research_Project-191391/Morteza/KCL_FACS/Scripts/QC/EPICV1/DNAm_QC_Header.html
params: 
...
SampleSheet<-read.csv(file = SampleSheet_file, stringsAsFactors = FALSE, sep = ",", na.strings = "")
...
if(file.exists(file= RGset_file)){
  ...
  message("Generating rgset object...")
  RGset <- read.metharray.exp(base = idatPath, targets = SampleSheet, force = TRUE,extended = F,verbose = F)
  save(RGset, file =RGset_file )
  message(paste("RGset created and saved as", RGset_file))
}
...
**Arrays ran by:** `r params$User_Array`, The University of Exeter Medical School

**Array used:** Illumina EPIC microarray v1.0

**QC done by:**`r params$User_QC`, The University of Exeter Medical School
...
The phenotype file containing the sample information was loaded into R. Then the methylation data for the `r nrow(SampleSheet)` samples were loaded into a methylumiset. This contains `r nrow(SampleSheet[SampleSheet$Sample_ID != "Meth_Control",])` `r params$Study_Name` and `r nrow(SampleSheet[SampleSheet$Sample_ID == "Meth_Control",])` fully methylated control samples.
...
msetEPIC<-msetEPIC[rownames(betas(msetEPIC)) %in% rownames(betas(msetEPIC.pf)),]#rownames(getBeta(RGset.pf)),]

#mark samples that fail the pfilter
pFilterPass<-colnames(betas(msetEPIC)) %in% colnames(betas(msetEPIC.pf))#colnames(RGset.pf)
...
counts=NA
if(tolower(SampleSheet$Organ)=="blood"){
  counts <- estimateCellCounts(RGset, compositeCellType ="Blood", cellTypes = c("CD8T","CD4T", "Bcell","Gran", "Mono","NK"), processMethod = "auto")
  save(counts, file=paste0(OutPrefix,".CellCounts.rdat"))
  message("CellCount created and saved in ", paste0(OutPrefix,".CellCounts.rdat"))
}

if(tolower(SampleSheet$Organ) == "brain"){

  counts <- estimateCellCounts(RGset, compositeCellType ="DLPFC")
  save(counts, file=paste0(OutPrefix,".CellCounts.rdat"))
  message("CellCount created and saved in ", paste0(OutPrefix,".CellCounts.rdat"))
}
